...
if (length(pos_Na)>0) {
  ...
  for (e in 1:length(Envs)){
    ...
    ETA=list(A=list(K=K.E , model="RKHS"),B=list(K=K.L, model="RKHS"), C=list(K=K.LE, model="RKHS"),D=list(X=EC_Scaled, model="BRR"))
    y2=y
    y2[pos_test_e]=NA
    model <-BGLR(
      ETA=ETA,
...
Saving_name=paste(name, "Pred_BGLR_CVO_Traits_E+G+GE.csv", sep="_")
